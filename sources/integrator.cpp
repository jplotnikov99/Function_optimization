...
double Integrator::adap_gauss_kronrod_15(double l, double r, const double err)
{
    double I1, I2, y[15];
    double h = (r - l) / 2;
    for (int i = 0; i < 15; i++)
    ...
    if (fabs((I1 - I2) / I1) < err)
    {
        return I1;
    }
    double m = (l + r) / 2;
    return adap_gauss_kronrod_15(l, m, err) + adap_gauss_kronrod_15(m, r, err);
}