...
size_t Optimizer::randomize_coeffs(FUNC &f)
{
    ...
    for (size_t i = 0; i < N_coeffs; i++)
        f.change_coeff(i, generate_random(coefficent_spaces[2 * space][i], coefficent_spaces[2 * space + 1][i]));
...
template <class FUNC>
void Optimizer::monte_carlo(FUNC &f, const size_t N, const size_t N_new_spaces)
{
