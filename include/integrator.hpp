...
private:
    ...
    template <class FUNC>
    double adap_gauss_kronrod_15(FUNC &f, double l, double r, const double appr, const size_t depth = 0,
...
template <class FUNC>
double Integrator::adap_gauss_kronrod_15(FUNC &f, double l, double r, const double appr, const size_t depth,
                                         const double err)
{
    double I1, I2, y[15];
    double h = (r - l) / 2;
    ...
    double m = (l + r) / 2;
    return adap_gauss_kronrod_15(f, l, m, appr, depth + 1, err) + adap_gauss_kronrod_15(f, m, r, appr, depth + 1, err);
}
