...
double Integrator::kronrod_61(const double l, const double r)
{
    ...
    for (size_t i = 0; i < 30; i++)
    {
        ...
        res += wkron_61[i] * (F->res(m + dx) + F->res(m - dx));
    }
    res += wkron_61[30] * F->res(m);
    return h * res;
}

double Integrator::adap_gauss_kronrod_15(double l, double r, const double appr, const double err)
{
    double I1, I2, y[15];
    double h = (r - l) / 2;
    for (int i = 0; i < 15; i++)
    {
        y[i] = F->res((kronx_15[i] + 1) * h + l);
    }
