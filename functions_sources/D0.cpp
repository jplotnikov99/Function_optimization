...
void dD0dw::set_x(const double new_x) { x = new_x; }

double dD0dw::operator()(const double u) {
    const double pre = 6 / (M_PI * M_PI);
    double w = x + (1 - u) / u;
    double expw = exp(w / 2);
    return 1 / (u * u) * pre * w * sqrt(w * w - x * x) /
...
double D0::approx(const double x) {
    return exp(-x) * (a1 - a2 +
                      (c[0] * pow(x, c[9]) + c[8] * pow(x, c[10])) /
                          (c[1] * pow(x, c[2]) + c[3] * pow(x, c[4]) +
                           c[5] * pow(x, c[6]) + c[7])) +
...
double D0::gradient(const double x) {
    vec1d grad(11);
    double den =
        c[1] * pow(x, c[2]) + c[3] * pow(x, c[4]) + c[5] * pow(x, c[6]) + c[7];
