...
vec1d Optimizer::get_opt_coeffs() { return opt_coeffs; }
...
double Optimizer::get_min_epsilon() { return min_epsilon; }
...
vec1d Optimizer::set_weight(const size_t space, const vec1d &constants,
                            const double eps) {
    ...
    for (size_t i = 0; i < N_coeffs; i++) {
        ...
        i2 = std::ceil((constants.at(i) - coefficent_spaces[2 * space][i]) *
                       ...
                       (coefficent_spaces[2 * space + 1][i] -
                        coefficent_spaces[2 * space][i])) -
             1;
        weights[i1 + i][i2] =
            weights[i1 + i][i2] == 0 ? eps : std::min(weights[i1 + i][i2], eps);
        res[i] = i2;
    }
    res[N_coeffs] = space;
    res[N_coeffs + 1] = eps;
